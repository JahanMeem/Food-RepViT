...
def get_args_parser():
    ...
    parser.add_argument('--batch-size', default=64, type=int)  # Reduced for Colab
    parser.add_argument('--epochs', default=100, type=int)  # Reduced epochs
    ...
    parser.set_defaults(repeated_aug=False)  # Disabled for custom dataset
    ...
    parser.add_argument('--distillation-type', default='none',  # Disabled by default
    ...
    # Dataset parameters - MODIFIED FOR FOOD DATASET
    parser.add_argument('--data-path', default='/content/food_dataset', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='FOOD', type=str,
                        help='Dataset type')
    ...
    parser.add_argument('--num_workers', default=2, type=int)  # Reduced for Colab
    ...
    parser.add_argument('--project', default='food-classification', type=str)
    return parser
...
def main(args):
    ...
    print("Building custom food dataset...")
    dataset_train, args.nb_classes = build_food_dataset(is_train=True, args=args)
    dataset_val, _ = build_food_dataset(is_train=False, args=args)
...
if __name__ == '__main__':
    ...
    if args.resume and not args.eval:
        ...
        args.output_dir = f"checkpoints/{args.model}/" + datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S')
        Path(args.output_dir).mkdir(parents=True, exist_ok=True)
    main(args)